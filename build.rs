use cmake::Config;
fn build_agent_ui() {
    let ui = std::path::Path::new("ztm/agent/gui");
    if cfg!(feature = "agent-ui") {
        if !ui.exists() {
            // run npm run build in the ztm/gui
